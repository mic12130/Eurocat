...
namespace Eurocat::Hmi::Track
{
		...
		POINT point, RECT area)
	{
		...
		else if (button == MouseButton::Middle)
		{
			...
			auto& pluginEnv = PluginEnvironment::Shared();
			auto fp = pluginEnv.GetPlugin().FlightPlanSelect(trackProfile.flightPlanId.value());
			auto fpProvider = FlightPlanDataProvider(fp, pluginEnv.AttributeForFlightPlan(fp));
